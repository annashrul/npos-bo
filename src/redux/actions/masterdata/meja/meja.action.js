...
export const FetchMejaAll = ()=>{
    return (dispatch) => {
        ...
        axios.get(HEADERS.URL+`meja?page=1&perpage=100`)
            .then(function(response){
                const data = response.data;
...
export const createMeja = (data,token) => {
    return (dispatch) => {
        dispatch(setLoading(true))
        ...
        axios.post(url, data)
            .then(function (response) {
                ...
                dispatch(setLoading(false));
                dispatch(FetchMeja(localStorage.getItem("page_meja")?localStorage.getItem("page_meja"):1,''));
