                ...
                <div className="card-body">
                    <div className="bg-transparent d-flex align-items-center justify-content-between mb-20">
                        ...
                        <span className="mr-5 ml-5" style={{display:this.state.show===false?'block':'none'}}></span>
                            <button type="button" class={"btn btn-link mr-2 animate__animated animate__faster " +(this.state.show===false?"animate__fadeInLeft":"animate__fadeInRight")} onClick={(e)=>this.handleBack(e)} style={{display:this.state.show===true?'block':'none'}}><i className="fa fa-chevron-left"></i>&nbsp;List Folders</button>
                        <div className="widgets-card-title">
                            <h5 className={"card-title mb-0 animate__animated "+(this.state.show===false?"animate__faster animate__fadeInLeft":"animate__fast animate__fadeInRight")}>{this.state.show===true?this.state.prevName:'List Folders'}</h5>
                        </div>
                        {/* </div> */}
                        <div>
                            <button type="button" disabled={this.state.mode === true?true:false} className="btn btn-light waves-effect waves-light ml-1" onClick={(e) => this.toggleMode(e)}><i className="fa fa-list"></i> </button>
                            <button type="button" disabled={this.state.mode === true?false:true} className="btn btn-light waves-effect waves-light ml-1" onClick={(e) => this.toggleMode(e)}><i className="fa fa-th"></i> </button>
                        </div>
                    </div>
                    <div className="row animate__animated animate__faster animate__slideInUp" style={{display:this.state.show===false?'flex':'none'}}>
                    {
                        !this.props.isLoading?(
                            typeof this.props.folder==='object'? this.props.folder.length>0?
                                                    ...
                                                    <div className="card-body">
                                                        <div className="widget-download-file d-flex align-items-center justify-content-between" key={i}>
                                                            <div className="d-flex align-items-center mr-3">
                                                                <button type="button" className="btn btn-warning btn-circle mr-3" onClick={(e) => this.handleOpen(e, v.path)}><i className="fa fa-folder"></i> </button>
                                ...
                                }) : <div className="col-md-12 mb-2">No folder found.</div> : <div className="col-md-12 mb-2">No folder found.</div>
                                ):<Preloader/>
                            }
                    </div>
                    <div className="row animate__animated animate__faster animate__slideInUp" style={{display:this.state.show===true?'flex':'none'}}>
                    {
                        !this.props.isLoading?(
                            typeof this.props.files==='object'? this.props.files.length>0?
                                                    ...
                                                    <div className="card-body">
                                                        <div className="widget-download-file d-flex align-items-center justify-content-between" key={i}>
                                                            ...
                                                            <div className="user-text-table">
                                                                ...
                                                                <a href={v.urls} target="_blank" className="download-link badge badge-success badge-pill" style={{padding:10+'px'}}>Download</a>
                                                        </div>
                                                    </div>
                                                </div>
                                                {/* </a> */}
                                ...
                                }) : <div className="col-md-12 mb-2">No files found.</div> : <div className="col-md-12 mb-2">No folder found.</div>
                                ):<Preloader/>
                            }
                    </div>
                </div>
