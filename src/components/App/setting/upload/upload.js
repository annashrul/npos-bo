            ...
            <Layout page="Product">
                ...
                <div className="card bg-boxshadow">
                    ...
                    <div className="user-important-data-info d-sm-flex align-items-center justify-content-between">
                        ...
                        <li><Link to="product" className="btn btn-link"><h5 className="mt-2"><i className="fa fa-chevron-left" /> Kembali</h5></Link></li>
                        </ul>
                    </div>
                    </div>
                </div>
                            ...
                            <div className="col-4">
                                <div className="card animate__animated animate__faster animate__fadeInUp" style={{display:this.state.toUpload==='uploadBarang'?'':'none'}}>
                                    <div className="card-body text-center">
                                        ...
                                        <button className="btn btn-rounded btn-primary" disabled={this.state.isUpload==='barang'?false:true} onClick={(e)=>this.handleSubmit(e)}><span class="spinner-border spinner-border-sm mr-2" style={{display:this.state.isLoad==='barang'?'':'none'}} role="status" aria-hidden="true"></span>{this.state.isLoad==='barang'?'Loading ...':'IMPORT'}</button>
                                    </div>
                                </div>
                                <div className="card animate__animated animate__fadeIn" style={{display:this.state.toUpload!=='uploadBarang'?'':'none'}}>
                                    ...
                                    <div className="card-body">
                                        ...
                                        <div className="d-flex align-items-center justify-content-between">
                                            ...
                                            <button onClick={(e)=>this.handleUpload(e,'uploadBarang')} class="btn btn-primary">Import File</button>
                                        </div>
                                    </div>
                                </div>
                            </div>
                            <div className="col-4">
                                <div className="card animate__animated animate__faster animate__fadeInUp" style={{display:this.state.toUpload==='uploadBarcode'?'':'none'}}>
                                    <div className="card-body text-center">
                                        ...
                                        <button className="btn btn-rounded btn-primary" disabled={this.state.isUpload==='barcode'?false:true} onClick={(e)=>this.handleSubmit(e)}><span class="spinner-border spinner-border-sm mr-2" style={{display:this.state.isLoad==='barcode'?'':'none'}} role="status" aria-hidden="true"></span>{this.state.isLoad==='barcode'?'Loading ...':'IMPORT'}</button>
                                    </div>
                                </div>
                                <div className="card animate__animated animate__fadeIn" style={{display:this.state.toUpload!=='uploadBarcode'?'':'none'}}>
                                    ...
                                    <div className="card-body">
                                        ...
                                        <p className="card-text text-justify">Pada template barcode, terdapat 5 kolom. Masing - masing kolom harus diisi dengan benar sesuai contoh yang terdapat dalam template tersebut. <del className="text-danger">BACA NOTE TERLEBIH DAHULU SEBELUM MELAKUKAN PENGISIAN TEMPLATE BARCODE</del></p>
                                        <div className="d-flex align-items-center justify-content-between">
                                            <a href={HEADERS.URL+"/templates/template_barcode.csv"} target="_blank" class="btn btn-info">Download Template</a>
                                            <button onClick={(e)=>this.handleUpload(e,'uploadBarcode')} class="btn btn-primary">Import File</button>
                                        </div>
                                    </div>
                                </div>
                            </div>
                            <div className="col-4">
                                <div className="card animate__animated animate__faster animate__fadeInUp" style={{display:this.state.toUpload==='uploadHarga'?'':'none'}}>
                                    <div className="card-body text-center">
                                        ...
                                        <button className="btn btn-rounded btn-primary" disabled={this.state.isUpload==='harga'?false:true} onClick={(e)=>this.handleSubmit(e)}><span class="spinner-border spinner-border-sm mr-2" style={{display:this.state.isLoad==='harga'?'':'none'}} role="status" aria-hidden="true"></span>{this.state.isLoad==='harga'?'Loading ...':'IMPORT'}</button>
                                    </div>
                                </div>
                                <div className="card animate__animated animate__fadeIn" style={{display:this.state.toUpload!=='uploadHarga'?'':'none'}}>
                                    ...
                                    <div className="card-body">
                                        ...
                                        <p className="card-text text-justify">Pada template harga, terdapat 10 kolom. Masing - masing kolom harus diisi dengan benar sesuai contoh yang terdapat dalam template tersebut. <del className="text-danger">BACA NOTE TERLEBIH DAHULU SEBELUM MELAKUKAN PENGISIAN TEMPLATE HARGA</del></p>
                                        <div className="d-flex align-items-center justify-content-between">
                                            <a href={HEADERS.URL+"/templates/template_harga.csv"} target="_blank" class="btn btn-info">Download Template</a>
                                            <button onClick={(e)=>this.handleUpload(e,'uploadHarga')} class="btn btn-primary">Import File</button>
                                        </div>
                                    </div>
                                </div>
                            </div>
