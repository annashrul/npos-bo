...
class Update extends Component{
    ...
    componentWillReceiveProps(nextProps){
        if (nextProps.datum !== undefined) {
            if (nextProps.datum !== this.state.before) {
                this.setState({
                    kd_trx: nextProps.datum.kd_trx,
                    before:nextProps.datum,
                    jumlah: nextProps.datum.jumlah,
                    keterangan: nextProps.datum.keterangan,
                    tgl: nextProps.datum.tgl,
                })
            }
        }
    }
    ...
    handleSubmit(e){
        ...
        if(this.state.jumlah===''||this.state.jumlah===undefined){
            ...
            return;
        } else if (this.state.keterangan===''||this.state.keterangan===undefined){
            err = Object.assign({}, err, {keterangan:"lokasi tidak boleh kosong"});
            this.setState({error: err});
            return;
        }
        else{
            this.props.dispatch(UpdateCashTrx(this.state.kd_trx,{
    ...
    render(){
        return (
            <WrapperModal isOpen={this.props.isOpen && this.props.type === "formUpdateKasTrx"} size="md">
                ...
                <form onSubmit={this.handleSubmit}>
                    <ModalBody>
                        ...
                        <div className="form-group">
                            <label>Jumlah</label>
                            <input type="number" className="form-control" name="jumlah" value={this.state.jumlah} onChange={this.handleChange}/>
                        ...
                        {/* <div className="form-group">
                            <label>Tanggal</label>
                            <input type="text" className="form-control" name="tgl" value={this.state.tgl} onChange={this.handleChange}/>
                            <div className="invalid-feedback"
                                 style={this.state.error.tgl !== "" ? {display: 'block'} : {display: 'none'}}>
                                {this.state.error.tgl}
                            </div>
                        </div> */}
