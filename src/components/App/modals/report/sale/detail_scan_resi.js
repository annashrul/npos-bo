...
class DetailScanresi extends Component {
  constructor(props) {
    super(props);
  ...
  render() {
    ...
    return (
      <WrapperModal
        isOpen={
          this.props.isOpen && this.props.type === "detailScanresi"
        }
        size="lg"
      >
        <ModalHeader toggle={this.toggle}>
          Detail laporan Scan Resi
        </ModalHeader>
        <ModalBody>
          <HeaderDetailCommon
            md="col-md-6"
...
const mapStateToProps = (state) => {
  return {
    data: state.scanResiLaporanReducer.dataGetDetailReport,
    isOpen: state.modalReducer,
    type: state.modalTypeReducer,
  };
};
export default connect(mapStateToProps)(DetailScanresi);
