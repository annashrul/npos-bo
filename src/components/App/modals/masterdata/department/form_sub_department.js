...
class FormSubDepartment extends Component {
  ...
  getProps(param) {
    ...
    if (param.dataDepartment !== undefined && param.dataDepartment !== []) {
      ...
      if (depProps !== undefined) {
        kode_departemen = depProps.filter(
          (res) => res.nama === param.detail.departement
        );
        this.setState({
  ...
  render() {
    return (
      <WrapperModal
        isOpen={this.props.isOpen && this.props.type === "formSubDepartment"}
        size="md"
      >
        <ModalHeader
          toggle={(e) => {
        ...
        >
          {this.props.detail.id === "" ? "Tambah" : "Ubah"} Sub Departemen
        </ModalHeader>
        <form onSubmit={this.handleSubmit}>
          <ModalBody>
            <SelectCommon
              label="Departemen"
              options={this.state.dept_data}
              callback={(res) => this.setState({ kode_dept: res.value })}
              dataEdit={this.state.kode_dept}
            />
