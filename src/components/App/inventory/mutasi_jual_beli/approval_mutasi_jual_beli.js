...
class ApprovalMutasiJualBeli extends Component{
    ...
    handlePageChange(pageNumber){
        this.props.dispatch(FetchApprovalMutation(pageNumber,this.state.kd_trx!==''?this.state.kd_trx:'',this.state.location!==''?this.state.location:'','TR'))
    }
    handleChange = (event) => {
        this.setState({ [event.target.name]: event.target.value });
    }
    HandleSearch(event){
        this.props.dispatch(FetchApprovalMutation(1,this.state.kd_trx,'','TR'));
    }
    toggleModal(e,kd_trx,perpage=20) {
        e.preventDefault();
        const bool = !this.props.isOpen;
        this.props.dispatch(ModalToggle(bool));
        this.props.dispatch(ModalType("formApprovalMutation"));
        localStorage.setItem("kd_trx_mutasi",kd_trx)
        this.props.dispatch(FetchApprovalMutationDetail(perpage,kd_trx))
    }
    toggle(e,index,lokasi){
        ...
        if(index!==this.state.indexOpen){
            this.props.dispatch(FetchApprovalMutation(1,'',lokasi,'TR'));
        }
    }
    render(){
        const {
                                ...
                                <div className="form-group">
                                    ...
                                    <div className="input-group">
                                        ...
                                        <div className="input-group-append">
                                            <button className={`btn btn-primary ${this.props.isLoading&&this.state.kd_trx!==''?'':'d-none'}`} type="button">
                                                <span className="spinner-border spinner-border-sm" role="status" aria-hidden="true" /><span className="sr-only">Loading...</span>
                                            </button>
                                        </div>
                                    </div>
                                </div>
                            ...
                            <div className="col-md-12">
                                <div>
                                    ...
                                    <CardBody style={{minHeight:'min-content',maxHeight:'400px',overflowX:'auto'}}>
                                        {
                                            typeof data === 'object' ? data.length > 0 ? data.map((w,j) => {
                                                return(
                                    ...
                                    // {
                                        this.state.location_data!==undefined? this.state.location_data.length > 0 ? this.state.location_data.map((v,i)=>{
                                            return(
                                                <div className="accordion" key={i}>
                                                    ...
                                                    <button className="btn btn-link btn-block text-left" onClick={(e)=>this.toggle(e,i,v.value)}>
                                                        ...
                                                        {this.props.isLoading&&this.state.indexOpen===i?<div className="spinner-border spinner-border-sm text-info" role="status"/>:<h5><i className={this.state.indexOpen===i?"fa fa-angle-up":"fa fa-angle-down"}></i></h5>}
                                                        </div>
                                                    </button>
