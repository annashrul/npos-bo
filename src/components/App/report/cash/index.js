...
class ReportCash extends Component {
  ...
  handleModal(type, obj) {
    if (type === "excel") {
      ...
      let setState = { isModalExport: true, startDate: getDate[0], endDate: getDate[1] };
  ...
  render() {
    const { last_page, total, per_page, current_page, total_kas, data } = this.props.cashReport;
    let total_perpage = 0;
    const head = [
    ...
    return (
      <Layout page="Laporan Kas">
          ...
          renderRow={
            <div className="col-md-4">
              <div className="row">
                ...
                <div className="col-6 col-xs-6 col-md-6">
                  <SelectCommon label="tipe" options={this.state.type_data} callback={(res) => this.handleSelect("type", res)} dataEdit={this.state.type} />
                </div>
              </div>
            </div>
          }
        ...
        />
        {this.props.isOpen && this.state.isModalExport ? <CashReportExcel tipe={this.state.type} startDate={this.state.startDate} endDate={this.state.endDate} kassa={this.state.kassa} /> : null}
