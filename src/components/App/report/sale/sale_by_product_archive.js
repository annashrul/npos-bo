...
class SaleByProductArchive extends Component{
    ...
    componentWillReceiveProps = (nextProps) => {
        ...
        localStorage.setItem('sort_sale_by_product_report',this.state.sort===''||this.state.sort===undefined?sort[0].kode:localStorage.sort_sale_by_product_report)
        localStorage.setItem('filter_sale_by_product_report',this.state.filter===''||this.state.filter===undefined?filter[0].kode:localStorage.filter_sale_by_product_report)
    }
    componentWillMount(){
    ...
    componentDidMount(){
        if (localStorage.location_sale_by_product_report !== undefined && localStorage.location_sale_by_product_report !== '') {
            this.setState({
                location: localStorage.location_sale_by_product_report
            })
        }

        if (localStorage.type_sale_by_product_report !== undefined && localStorage.type_sale_by_product_report !== '') {
            this.setState({
                type: localStorage.type_sale_by_product_report
            })
        }
        if (localStorage.any_sale_by_product_report !== undefined && localStorage.any_sale_by_product_report !== '') {
            this.setState({
                any: localStorage.any_sale_by_product_report
            })
        }
        if (localStorage.date_from_sale_by_product_report !== undefined && localStorage.date_from_sale_by_product_report !== null) {
            this.setState({
                startDate: localStorage.date_from_sale_by_product_report
            })
        }
        if (localStorage.date_to_sale_by_product_report !== undefined && localStorage.date_to_sale_by_product_report !== null) {
            this.setState({
                endDate: localStorage.date_to_sale_by_product_report
            })
        }
        if (localStorage.sort_sale_by_product_report !== undefined && localStorage.sort_sale_by_product_report !== null) {
            this.setState({sort: localStorage.sort_sale_by_product_report})
        }
        if (localStorage.filter_sale_by_product_report !== undefined && localStorage.filter_sale_by_product_report !== null) {
            this.setState({filter: localStorage.filter_sale_by_product_report})
        }
        if (localStorage.status_sale_by_product_report !== undefined && localStorage.status_sale_by_product_report !== null) {
    ...
    handleSearch(e){
        e.preventDefault();
        localStorage.setItem("any_sale_by_product_report",this.state.any_sale_by_product_report);
    ...
    checkingParameter(pageNumber){
        ...
        // }
        if(sort!==undefined&&sort!==null&&sort!==''){
            if(where!==''){where+='&'}where+=`sort=${sort}`;
        }
    ...
    handleDetail(e,kode,kd_brg,nm_brg,deskripsi,satuan,qty_jual,gross_sales,diskon_item,tax,service,toko,tgl){
        ...
        this.props.dispatch(FetchReportDetailSaleByProduct(kode,1,dateFrom===null?this.state.startDate:dateFrom,dateTo===null?this.state.endDate:dateTo));
    }
    ...
    toggleModal(e,total) {
        ...
        this.props.dispatch(FetchReportSaleByProductExcel(1,this.state.where_data,total));
    }
            ...
            <Layout page="Laporan Arsip Penjualan">
                <div className="card">
                    <div className="card-header">
                        <h5>Laporan Arsip Penjualan By Barang</h5>
                    </div>
                    <div className="card-body">
                        <div className="row">
                                ...
                                <div className="row">
                                    <div className="col-6 col-xs-6 col-md-3">
                                        <div className="form-group">
                                            ...
                                            <DateRangePicker style={{display:'unset'}} ranges={rangeDate} alwaysShowCalendars={true} onEvent={this.handleEvent}>
                                                <input type="text" className="form-control" name="date_sale_by_product_report" value={`${this.state.startDate} to ${this.state.endDate}`} style={{padding: '9px',fontWeight:'bolder'}}/>
                                            </DateRangePicker>
                                    ...
                                    <div className="col-6 col-xs-6 col-md-3">
                                        <div className="form-group">
                                            ...
                                            <input type="text" name="any_sale_by_product_report" placeholder="Kode/Barcode/Nama/Deskipsi" className="form-control form-control-lg" value={this.state.any_sale_by_product_report} onChange={(e)=>this.handleChange(e)}/>
                                        </div>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div className="col-md-12">
                                ...
                                <SaleByProductReportExcel startDate={this.state.startDate} endDate={this.state.endDate} location={this.state.location} />
                            </div>
                        </div>
                    </div>
                </div>
