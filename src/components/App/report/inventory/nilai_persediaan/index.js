...
class NilaiPersediaanReport extends Component {
  ...
  render() {
    ...
    // bukaHarga && head.push({ ...colOrRow, label: "Total Harga Jual", width: "1%" });
    let rowSpan = [];

    if (setHarga > 1 && propsUser.nama_harga !== undefined) {
      for (let i = 0; i < setHarga; i++) bukaHarga && rowSpan.push({ label: propsUser.nama_harga[`harga${i + 1}`] });
      // for (let i = 0; i < setHarga; i++) bukaHarga && rowSpan.push({ label: propsUser.nama_harga[`harga${i + 1}`] });
    }
          ...
          footer={[
            {
              data: [
                ...
                // {
                //   colSpan: 1,
                //   label: parseToRp(total_nilai_persediaan_total_harga_1),
                //   className: `text-right ${bukaSetHarga[0] !== undefined && bukaHarga ? "" : "dNone"}`,
                // },
                // {
                //   colSpan: 1,
                //   label: parseToRp(total_nilai_persediaan_total_harga_2),
                //   className: `text-right ${bukaSetHarga[1] !== undefined && bukaHarga ? "" : "dNone"}`,
                // },
                // {
                //   colSpan: 1,
                //   label: parseToRp(total_nilai_persediaan_total_harga_3),
                //   className: `text-right ${bukaSetHarga[2] !== undefined && bukaHarga ? "" : "dNone"}`,
                // },
                // {
                //   colSpan: 1,
                //   label: parseToRp(total_nilai_persediaan_total_harga_4),
                //   className: `text-right ${bukaSetHarga[3] !== undefined && bukaHarga ? "" : "dNone"}`,
                // },
              ],
            },
