...
import {
  FetchOpname,
  FetchOpnameExcel,
} from "redux/actions/inventory/opname.action";
...
class OpnameReport extends Component {
  ...
  componentDidMount() {
    if (
      localStorage.location_opname_report !== undefined &&
      localStorage.location_opname_report !== ""
    ) {
      this.setState({ location: localStorage.location_opname_report });
    }
    if (
      localStorage.any_opname_report !== undefined &&
      localStorage.any_opname_report !== ""
    ) {
      this.setState({ any: localStorage.any_opname_report });
    }
    if (
      localStorage.date_from_opname_report !== undefined &&
      localStorage.date_from_opname_report !== null
    ) {
      this.setState({ startDate: localStorage.date_from_opname_report });
    }
    if (
      localStorage.date_to_opname_report !== undefined &&
      localStorage.date_to_opname_report !== null
    ) {
      this.setState({ endDate: localStorage.date_to_opname_report });
    }
    if (
      localStorage.sort_opname_report !== undefined &&
      localStorage.sort_opname_report !== null
    ) {
      this.setState({ sort: localStorage.sort_opname_report });
    }
    if (
      localStorage.filter_opname_report !== undefined &&
      localStorage.filter_opname_report !== null
    ) {
      this.setState({ filter: localStorage.filter_opname_report });
    }
    if (
      localStorage.status_opname_report !== undefined &&
      localStorage.status_opname_report !== null
    ) {
  ...
  componentWillReceiveProps = (nextProps) => {
    ...
    localStorage.setItem(
      "status_opname_report",
      this.state.status === "" || this.state.status === undefined
        ? status[0].kode
        : localStorage.status_opname_report
    );
    localStorage.setItem(
      "sort_opname_report",
      this.state.sort === "" || this.state.sort === undefined
        ? sort[0].kode
        : localStorage.sort_opname_report
    );
    localStorage.setItem(
      "filter_opname_report",
      this.state.filter === "" || this.state.filter === undefined
        ? filter[0].kode
        : localStorage.filter_opname_report
    );
  };
    ...
    return (
      <Layout page="Laporan Opname">
        ...
        <table
          ...
          id="report_opname_to_excel"
          style={{ display: "none" }}
        >
          <thead className="bg-light">
            <tr>
              <th className="text-black" colSpan={11}>
          ...
          {
            <tbody>
              {typeof this.props.opnameReportExcel.data === "object" ? (
                this.props.opnameReportExcel.data.length > 0 ? (
                  this.props.opnameReportExcel.data.map((v, i) => {
                    return (
                      <tr key={i}>
                        ...
                        <td style={columnStyle}>
                          {v.status === "0"
                            ? statusQ("danger", "Belum Opname")
                            : v.status === "1"
                            ? statusQ("warning", "Sudah Opname")
                            : ""}
                        </td>
                      </tr>
                    );
                      ...
                      return (
                        <tr key={i}>
                          ...
                          <td style={columnStyle}>
                            {
                              v.status === "0"
                                ? statusQ("warning", "Belum Opname")
                                : v.status === "1"
                                ? statusQ("success", "Sudah Opname")
                                : statusQ("danger", "Cancel Opname")
